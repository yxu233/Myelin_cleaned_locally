...
remove_nets = 'N'; % background subtraction for O4
mag = 'N';
...
while (moreTrials == 'Y')
    ...
    for fileNum = 1 : load_five: numfids
        ...
        intensityValueDAPI = im2double(gray(:,:,3));
        O4_original = im2double(gray(:,:,1));
        ...
        %% Decide if want to subtract out background nanofibers
        if nanoYN == 1
            nanoF_im = adapthisteq(greenImage);   % adjust
%            if mag == 'Y'
...
%             end
        ...
        %% LOOP
        counter = 0;
        intensityValueDAPI = im2double(DAPIimage);
        O4_original = im2double(redImage);
        MBP_im = im2double(greenImage);
        ...
        %% ONLY USE IF REAL BAD FLUORESCENCE???
        ...
        %% SUBTRACT OUT IF HAVE NANOFIBERS:
        if nanoYN
            O4_im_ridges_adapted(nanoF_im == 0) = 0;   % SUBTRACT THEM
        ...
        if nanoYN
            figure(2);
            filename = strcat('Result', erase(name, '*'), num2str(fileNum_sav),  '_', filename_raw, '_', num2str(counter), '10) nanofibers BW') ;
            print(filename,'-dpng')
            hold off;
        end
